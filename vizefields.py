...
def electricfield(qs, rs, p, eps=1):
  """
  ...
  """
  rs = np.asarray(rs) #everything is better as an array, right?
  p = np.asarray(p)
  field = [0, 0]
...
def plotelectricfieldmag(qs, rs, width = 2, res= 50):
  """
	creates a plot of the magnitude of the electric field calculated in electricfield()
  """
...
def plotelectricfieldvec(qs, rs, width =2, res=50):
  """
  uses electricfield() to plot a vector field
  """
  x, y = np.meshgrid(np.linspace(-width, width, res), np.linspace(-width, width, res))
  fieldgrid = np.zeros((res, res))
  field = 0
